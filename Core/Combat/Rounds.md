...
Besides the main numeric cards, there are some unique cards that have different properties and values, as shown in the table below.


| Card  | Value    | Property                                                               |
| ----- | -------- | ---------------------------------------------------------------------- |
| Ace   | 1        | Your highest value dice is locked to a value of 1 for the round        |
| Jack  | 11       | Gain one d6 that is only able to be used as a reaction                 |
| Queen | 12       | Gain one d6 that is only able to be used as an action                  |
| King  | 13       | Gain two d6 that is able to be used as either an action or reaction    |
| Joker | Variable | You are able to select where you go in the initiative order this round |
In the case that two people have cards with the same numeric value, suits in this list from to ascending go first in that same numeric value:
...
> Example: if two people have a card with a numeric value of 6, Person A's suit is a heart and Person B's suit is a spade, Person A goes first, then Person B goes next.
